...
2. Run the bundled script:
   ...
   - Use `--format brief-zh` for the same structure with Chinese labels.
   - For batch usage: `json` returns `{count, results}`, `json-compact` returns JSONL, and human-readable formats emit multiple blocks with separators.
